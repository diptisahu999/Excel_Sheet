...
def user_list(request):
    if request.method == 'GET':
        ...
        if uses_serializer.is_valid():
            ...
            return Response({"data":"true","status_code": 200,"access_token":str(token), "message": "User Added Successfully", "response":uses_serializer.data},status=status.HTTP_201_CREATED)
...
def get_tower_list(request):
    if request.method == 'GET': 
        ...
        result.sort(key=lambda x: x['tower_id'])
                    # name = i.tower_name
                    # id = i.id
...
def get_tower_device_data(request):
    ...
    if request.method == 'GET': 
        ...
        data = request.data
        print(data,"data recv from client")
        device_type = data['device_type']
...
def get_area_device_data(request):
    if request.method == 'GET': 
        ...
        return Response({"data":"true","status_code": 200, "message": "User Areas and Devices Access list ","response":{"sub_area_data":result,"device_data":devices_data}}, status=status.HTTP_200_OK)
